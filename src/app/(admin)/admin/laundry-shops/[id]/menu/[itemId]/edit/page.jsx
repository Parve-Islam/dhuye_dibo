// app/admin/laundry-shops/[id]/menu/[itemId]/edit/page.jsx
'use client';
...
export default function EditMenuItem() {
  ...
  useEffect(() => {
    async function fetchShopDetails() {
      try {
        ...
        const item = data.laundryShop.menu.find(item => item._id === params.itemId);
        if (!item) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/laundry-shops/${params.id}/menu/${params.itemId}`, {
        method: 'PUT',
        headers: {
  ...
  if (!shop || !menuItem) return <div className="p-8">Item not found</div>;

  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Edit Service - {shop.name}</h1>
        <Link 
      ...
      <form onSubmit={handleSubmit} className="bg-white shadow-md rounded-lg p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2">
            ...
            <textarea
             // app/admin/laundry-shops/[id]/menu/[itemId]/edit/page.jsx (continuation)
             name="description"
